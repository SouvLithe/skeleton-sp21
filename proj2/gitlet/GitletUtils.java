...
public class GitletUtils {
    ...
    public static void branch(String[] args) {
        judgeCommand(args, 1);
        Branch branch = new Branch(args[1], readHEADContent());
        branch.updateBranch();
    }
    ...
    public static void merge(String[] args) {
        judgeCommand(args, 1);
        Branch cur = readHEADAsBranch();
        Branch branch = Branch.readBranch(args[1]);
        if (branch == null) {
            exit("A branch with that name does not exist.");
        }
        if (branch.toString().equals(cur.toString())) {
            exit("Cannot merge a branch with itself.");
        }
        if (!readStagingArea().isCommited()) {
            exit("You have uncommitted changes.");
        }
        untrackedExist();
        Merge.merge(cur, branch);
    }
    ...
    public static void push(String[] args) {
        ...
        if (!r.isExists(remoteName) || !r.getRemote(remoteName).exists()) {
            HelperMethods.exit("Remote directory not found.");
        }
    ...
    public static void fetch(String[] args) {
        judgeCommand(args, 2);
        Remote remote = readRemotes();
        String remoteName = args[1];
        String branchName = args[2];
        if (!remote.isExists(remoteName) || !remote.getRemote(remoteName).exists()) {
            HelperMethods.exit("Remote directory not found.");
        }
        remote.fetch(remoteName, Branch.readBranch(branchName, getRemoteBranchDir(remoteName)));
    }
